...
#define KADEMLIA_VERSION6_49aBETA \
    0x06  // -0.49aBETA1, needs to support: OP_FWCHECKUDPREQ (!), obfuscation, direct callbacks, source type 6, UDP
          // firewallcheck
...
struct kad_id : public md4_hash {
    enum { kad_total_bits = md4_hash::size * 8 };

    kad_id();
    kad_id(const md4_hash& h);
    kad_id(const md4_hash::md4hash_container&);

    operator md4_hash() { return md4_hash(m_hash); }
...
struct kad_nodes_dat {
    ...
    void save(Archive& ar) {
        ...
        ar& num_contacts;
        for (std::list<kad_entry>::const_iterator itr = contacts_container.begin(); itr != contacts_container.end();
             ++itr) {
            ar&* itr;
        }
    }
...
*/
template <typename T>
struct transaction_identifier;

template <>
struct transaction_identifier<kad2_ping> {
    static const uint16_t id = 'p';
};
template <>
struct transaction_identifier<kad2_pong> {
    static const uint16_t id = 'p';
};

// bootstrap request/response identifiers
template <>
struct transaction_identifier<kad2_bootstrap_req> {
    static const uint16_t id = 'b';
};
template <>
struct transaction_identifier<kad2_bootstrap_res> {
    static const uint16_t id = 'b';
};

// hello request/response
template <>
struct transaction_identifier<kad2_hello_req> {
    static const uint16_t id = 'h';
};
template <>
struct transaction_identifier<kad2_hello_res> {
    static const uint16_t id = 'h';
};

// kademlia request/response
template <>
struct transaction_identifier<kademlia2_req> {
    static const uint16_t id = 'l';
};
template <>
struct transaction_identifier<kademlia2_res> {
    static const uint16_t id = 'l';
};
...
template <>
struct transaction_identifier<kad2_search_key_req> {
    static const uint16_t id = 's';
};
template <>
struct transaction_identifier<kad2_search_notes_req> {
    static const uint16_t id = 's';
};
template <>
struct transaction_identifier<kad2_search_sources_req> {
    static const uint16_t id = 's';
};

// we have one packet format for all three requests above
template <>
struct transaction_identifier<kad2_search_res> {
    static const uint16_t id = 's';
};

// firewalled
template <>
struct transaction_identifier<kad_firewalled_req> {
    static const uint16_t id = 'f';
};
template <>
struct transaction_identifier<kad_firewalled_res> {
    static const uint16_t id = 'f';
};
}
