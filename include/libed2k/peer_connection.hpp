...
struct pending_block {
    ...
    bool operator==(const pending_block& b) {
        return b.skipped == skipped && b.block == block && b.not_wanted == not_wanted && b.timed_out == timed_out;
    }
...
struct has_block {
    ...
    bool operator()(const pending_block& pb) const { return pb.block == block; }
};
...
class peer_connection : public base_connection {
   public:
    ...
    peer_connection(aux::session_impl& ses, boost::weak_ptr<transfer>, boost::shared_ptr<tcp::socket> s,
                    const tcp::endpoint& remote, peer* peerinfo);

    // with this constructor we have been contacted and we still don't
    // know which transfer the connection belongs to
    peer_connection(aux::session_impl& ses, boost::shared_ptr<tcp::socket> s, const tcp::endpoint& remote,
                    peer* peerinfo);
    ...
    int bandwidth_throttle(int channel) const { return m_bandwidth_channel[channel].throttle(); }
   ...
   private:
    ...
    int request_upload_bandwidth(bandwidth_channel* bwc1, bandwidth_channel* bwc2 = 0, bandwidth_channel* bwc3 = 0,
                                 bandwidth_channel* bwc4 = 0);
    int request_download_bandwidth(bandwidth_channel* bwc1, bandwidth_channel* bwc2 = 0, bandwidth_channel* bwc3 = 0,
                                   bandwidth_channel* bwc4 = 0);
    ...
    void receive_data();
    void on_disk_write_complete(int ret, disk_io_job const& j, peer_request req, boost::shared_ptr<transfer> t);
    ...
    template <typename Struct>
    void on_compressed_part(const error_code& error);

    template <typename T>
    void defer_write(const T& t);
    template <typename T>
