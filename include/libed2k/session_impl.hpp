...
class session_impl : public session_impl_base {
   ...
   public:
    ...
    session_impl(const fingerprint& id, const char* listen_interface, const session_settings& settings);
    ~session_impl();
    ...
    void on_accept_connection(boost::shared_ptr<tcp::socket> const& s, boost::weak_ptr<tcp::acceptor> listener,
                              error_code const& e);
    ...
    enum { source_dht = 1, source_peer = 2, source_tracker = 4, source_router = 8 };
    ...
    bool has_peer(const peer_connection* p) const {
        return std::find_if(m_connections.begin(), m_connections.end(),
                            boost::bind(&boost::intrusive_ptr<peer_connection>::get, _1) == p) != m_connections.end();
    }
    ...
    void set_external_address(address const& ip, int source_type, address const& source);
...
// return m_network_thread == pthread_self();
    ...
    void on_dht_router_name_lookup(error_code const& e, tcp::resolver::iterator host);
    ...
    */
    ...
    void on_find_dht_source(const md4_hash& hash, uint8_t type, client_id_type ip, uint16_t port,
