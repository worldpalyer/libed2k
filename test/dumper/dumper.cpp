...
int main(int argc, char* argv[]) {
    ...
    try {
        libed2k::search_request sr =
            libed2k::generateSearchRequest(0, 0, 0, 0, "", "", "", 0, 0, libed2k::convert_from_native(src));

        for (libed2k::search_request::const_iterator itr = sr.begin(); itr != sr.end(); ++itr) {
            DBG("Operand: {" << (itr->isLogic() ? "true" : "false") << "} value: {"
                             << libed2k::sre_operation2string(itr->getOperator()) << "} str: {" << itr->getStrValue()
                             << "}");
        }
