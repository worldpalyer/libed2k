...
#define SET(n) (*(const boost::uint32_t*)&ptr[(n)*4])
#define GET(n) SET(n)
#else
#define SET(n)                                                                                \
    (ctx->block[(n)] = (boost::uint32_t)ptr[(n)*4] | ((boost::uint32_t)ptr[(n)*4 + 1] << 8) | \
                       ((boost::uint32_t)ptr[(n)*4 + 2] << 16) | ((boost::uint32_t)ptr[(n)*4 + 3] << 24))
...
namespace libed2k {
const md4_hash md4_hash::terminal() {
	return md4_hash::fromString("31D6CFE0D16AE931B73C59D7E0C089C0");
}
const md4_hash md4_hash::libed2k() {
	return md4_hash::fromString("31D6CFE0D14CE931B73C59D7E0C04BC0");
}
const md4_hash md4_hash::emule() {
	return md4_hash::fromString("31D6CFE0D10EE931B73C59D7E0C06FC0");
}
const md4_hash md4_hash::invalid() {
	return md4_hash::fromString("00000000000000000000000000000000");
}
...
md4_hash md4_hash::fromHashset(const std::vector<md4_hash>& hashset) {
    ...
    if (hashset.size() > 1) {
        MD4_CTX ctx;
        MD4_Init(&ctx);
        MD4_Update(&ctx, reinterpret_cast<const boost::uint8_t*>(&hashset[0]), hashset.size() * MD4_DIGEST_LENGTH);
...
md4_hash::md4_hash(const std::vector<boost::uint8_t>& vHash) {
    ...
    for (size_t i = 0; i < nSize; i++) m_hash[i] = vHash[i];
}

md4_hash::md4_hash(const md4hash_container& container) { memcpy(m_hash, container, MD4_DIGEST_LENGTH); }

md4_hash::md4_hash(const char* s) {
    if (s == 0)
        clear();
    else
        std::memcpy(m_hash, s, size);
}
...
bool md4_hash::operator==(md4_hash const& n) const { return std::equal(n.m_hash, n.m_hash + number_size, m_hash); }
...
md4_hash md4_hash::operator~() const {
    md4_hash ret;
    for (int i = 0; i < number_size; ++i) ret.m_hash[i] = ~m_hash[i];
    return ret;
}
...
md4_hash& md4_hash::operator&=(md4_hash const& n) {
    for (int i = 0; i < number_size; ++i) m_hash[i] &= n.m_hash[i];
    return *this;
}
...
bool md4_hash::defined() const {
    int sum = 0;
    for (size_t i = 0; i < MD4_DIGEST_LENGTH; ++i) sum |= m_hash[i];
    return sum != 0;
}

std::string md4_hash::toString() const {
    static const char c[MD4_DIGEST_LENGTH] = {'\x30', '\x31', '\x32', '\x33', '\x34', '\x35', '\x36', '\x37',
...
void md4_hash::dump() const { DBG("md4_hash::dump " << toString().c_str()); }
}
