...
inline void throw_invalid_pc_handle() { throw libed2k_exception(errors::invalid_transfer_handle); }

inline void throw_null_session_pointer() { throw libed2k_exception(errors::session_pointer_is_null); }
#endif

peer_connection_handle::peer_connection_handle() : m_pses(NULL) {}

peer_connection_handle::peer_connection_handle(boost::intrusive_ptr<peer_connection> pc, aux::session_impl* pses)
    : m_pses(pses) {
    if (pc) {
...
void peer_connection_handle::get_shared_files() const { LIBED2K_PC_FORWARD_NOLOCK(request_shared_files()) }

void peer_connection_handle::get_shared_directories() const { LIBED2K_PC_FORWARD_NOLOCK(request_shared_directories()) }
...
md4_hash peer_connection_handle::get_hash() const { LIBED2K_PC_FORWARD_RETURN(get_connection_hash(), md4_hash()) }
