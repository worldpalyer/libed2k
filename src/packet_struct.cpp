...
const char* toString(search_request_entry::SRE_Operation so) {
    static const char* chOperations[] = {"AND", "OR", "NOT"};
...
net_identifier::net_identifier(const tcp::endpoint& ep) : m_nIP(address2int(ep.address())), m_nPort(ep.port()) {}
...
void net_identifier::dump() const { DBG("{" << to_string() << "}"); }
...
shared_file_entry::shared_file_entry(const md4_hash& hFile, boost::uint32_t nFileId, boost::uint16_t nPort)
    : m_hFile(hFile), m_network_point(nFileId, nPort) {}
...
client_hello_answer::client_hello_answer(const md4_hash& client_hash, const net_identifier& np,
                                         const net_identifier& sp, const std::string& client_name,
                                         const std::string& program_name, boost::uint32_t version) {
...
void client_hello_answer::dump() const {
    DBG("hello answer {client_hash: " << m_hClient << ", client_ip: " << int2ipstr(m_network_point.m_nIP)
...
client_hello::client_hello(const md4_hash& client_hash, const net_identifier& np, const net_identifier& sp,
                           const std::string& client_name, const std::string& program_name, boost::uint32_t version)
    : client_hello_answer(client_hash, np, sp, client_name, program_name, version), m_nHashLength(MD4_DIGEST_LENGTH) {}
...
search_request_entry::search_request_entry(const std::string& strMetaTagName, boost::uint8_t nOperator,
...
bool search_request_entry::isLogic() const { return (m_type == SEARCH_TYPE_BOOL && m_operator <= SRE_NOT); }

bool search_request_entry::isOperator() const { return (m_type == SEARCH_TYPE_BOOL); }

const std::string& search_request_entry::getStrValue() const { return (m_strValue); }

boost::uint32_t search_request_entry::getInt32Value() const { return (m_nValue32); }

boost::uint64_t search_request_entry::getInt64Value() const { return (m_nValue64); }

boost::uint8_t search_request_entry::getOperator() const { return (m_operator); }
...
std::string sre_operation2string(boost::uint8_t oper) {
    static const char* s[] = {"SRE_AND", "SRE_OR", "SRE_NOT", "SRE_OBR", "SRE_CBR", "SRE_END"};
...
misc_options::misc_options() { load(0); }

misc_options::misc_options(boost::uint32_t opts) { load(opts); }
...
void misc_options2::load(boost::uint32_t opts) { m_options = opts; }

bool misc_options2::support_captcha() const { return ((m_options >> CAPTHA_OFFSET) & 0x01); }

bool misc_options2::support_source_ext2() const { return ((m_options >> SRC_EXT_OFFSET) & 0x01); }

bool misc_options2::support_ext_multipacket() const { return ((m_options >> MULTIP_OFFSET) & 0x01); }

bool misc_options2::support_large_files() const { return ((m_options >> LARGE_FILE_OFFSET) & 0x01); }
...
boost::uint32_t misc_options2::generate() const { return (m_options); }

boost::uint32_t misc_options::generate() const {
    return ((m_nAICHVersion << ((4 * 7) + 1)) | (m_nUnicodeSupport << 4 * 7) | (m_nUDPVer << 4 * 6) |
            (m_nDataCompVer << 4 * 5) | (m_nSupportSecIdent << 4 * 4) | (m_nSourceExchange1Ver << 4 * 3) |
            (m_nExtendedRequestsVer << 4 * 2) | (m_nAcceptCommentVer << 4 * 1) | (m_nNoViewSharedFiles << 1 * 2) |
            (m_nMultiPacket << 1 * 1) | (m_nSupportsPreview << 1 * 0));
}
...
std::string kad2string(int op) {
    switch (op) {
        case KADEMLIA2_BOOTSTRAP_REQ:
            return "KADEMLIA2_BOOTSTRAP_REQ";
        case KADEMLIA2_BOOTSTRAP_RES:
            return "KADEMLIA2_BOOTSTRAP_RES";
        case KADEMLIA2_HELLO_REQ:
            return "KADEMLIA2_HELLO_REQ";
        case KADEMLIA2_HELLO_RES:
            return "KADEMLIA2_HELLO_RES";
        case KADEMLIA2_REQ:
            return "KADEMLIA2_REQ";
        case KADEMLIA2_HELLO_RES_ACK:
            return "KADEMLIA2_HELLO_RES_ACK";
        case KADEMLIA2_RES:
            return "KADEMLIA2_RES";
        case KADEMLIA2_SEARCH_KEY_REQ:
            return "KADEMLIA2_SEARCH_KEY_REQ";
        case KADEMLIA2_SEARCH_SOURCE_REQ:
            return "KADEMLIA2_SEARCH_SOURCE_REQ";
        case KADEMLIA2_SEARCH_NOTES_REQ:
            return "KADEMLIA2_SEARCH_NOTES_REQ";
        case KADEMLIA2_SEARCH_RES:
            return "KADEMLIA2_SEARCH_RES";
        ...
        case KADEMLIA_FIREWALLED2_REQ:
            return "KADEMLIA_FIREWALLED2_REQ";
        case KADEMLIA2_PING:
            return "KADEMLIA2_PING";
        case KADEMLIA2_PONG:
            return "KADEMLIA2_PONG";
        case KADEMLIA2_FIREWALLUDP:
            return "KADEMLIA2_FIREWALLUDP";
        default:
            break;
    }
    ...
    switch (op) {
        case KADEMLIA_BOOTSTRAP_REQ_DEPRECATED:
            return "KADEMLIA_BOOTSTRAP_REQ_DEPRECATED";  // = 0x00, // <PEER (sender) [25]>
        case KADEMLIA_BOOTSTRAP_RES_DEPRECATED:
            return "KADEMLIA_BOOTSTRAP_RES_DEPRECATED";  // = 0x08, // <CNT [2]> <PEER [25]>*(CNT)
        case KADEMLIA_HELLO_REQ_DEPRECATED:
            return "KADEMLIA_HELLO_REQ_DEPRECATED";  // = 0x10, // <PEER (sender) [25]>
        case KADEMLIA_HELLO_RES_DEPRECATED:
            return "KADEMLIA_HELLO_RES_DEPRECATED";  // = 0x18, // <PEER (receiver) [25]>
        case KADEMLIA_REQ_DEPRECATED:
            return "KADEMLIA_REQ_DEPRECATED";  // = 0x20, // <TYPE [1]> <HASH (target) [16]> <HASH (receiver) 16>
        case KADEMLIA_RES_DEPRECATED:
            return "KADEMLIA_RES_DEPRECATED";  // = 0x28, // <HASH (target) [16]> <CNT> <PEER [25]>*(CNT)
        case KADEMLIA_SEARCH_REQ:
            return "KADEMLIA_SEARCH_REQ";  // = 0x30, // <HASH (key) [16]> <ext 0/1 [1]> <SEARCH_TREE>[ext]
        // UNUSED               = 0x31, // Old Opcode, don't use.
        case KADEMLIA_SEARCH_NOTES_REQ:
            return "KADEMLIA_SEARCH_NOTES_REQ";  // = 0x32, // <HASH (key) [16]>
        case KADEMLIA_SEARCH_RES:
            return "KADEMLIA_SEARCH_RES";  // = 0x38, // <HASH (key) [16]> <CNT1 [2]> (<HASH (answer) [16]> <CNT2 [2]>
                                           // <META>*(CNT2))*(CNT1)
        // UNUSED               = 0x39, // Old Opcode, don't use.
        case KADEMLIA_SEARCH_NOTES_RES:
            return "KADEMLIA_SEARCH_NOTES_RES";  // = 0x3A, // <HASH (key) [16]> <CNT1 [2]> (<HASH (answer) [16]> <CNT2
                                                 // [2]>
                                                 // <META>*(CNT2))*(CNT1)
        case KADEMLIA_PUBLISH_REQ:
            return "KADEMLIA_PUBLISH_REQ";  // = 0x40, // <HASH (key) [16]> <CNT1 [2]> (<HASH (target) [16]> <CNT2 [2]>
                                            // <META>*(CNT2))*(CNT1)
        // UNUSED               = 0x41, // Old Opcode, don't use.
        case KADEMLIA_PUBLISH_NOTES_REQ_DEPRECATED:
            return "KADEMLIA_PUBLISH_NOTES_REQ_DEPRECATED";  // = 0x42, // <HASH (key) [16]> <HASH (target) [16]> <CNT2
                                                             // [2]>
                                                             // <META>*(CNT2))*(CNT1)
        case KADEMLIA_PUBLISH_RES:
            return "KADEMLIA_PUBLISH_RES";  // = 0x48, // <HASH (key) [16]>
        // UNUSED               = 0x49, // Old Opcode, don't use.
        case KADEMLIA_PUBLISH_NOTES_RES_DEPRECATED:
            return "KADEMLIA_PUBLISH_NOTES_RES_DEPRECATED";  // = 0x4A, // <HASH (key) [16]>
        case KADEMLIA_FIREWALLED_REQ:
            return "KADEMLIA_FIREWALLED_REQ";  // = 0x50, // <TCPPORT (sender) [2]>
        case KADEMLIA_FINDBUDDY_REQ:
            return "KADEMLIA_FINDBUDDY_REQ";  // = 0x51, // <TCPPORT (sender) [2]>
        case KADEMLIA_CALLBACK_REQ:
            return "KADEMLIA_CALLBACK_REQ";  // = 0x52, // <TCPPORT (sender) [2]>
        case KADEMLIA_FIREWALLED_RES:
            return "KADEMLIA_FIREWALLED_RES";  // = 0x58, // <IP (sender) [4]>
        case KADEMLIA_FIREWALLED_ACK_RES:
            return "KADEMLIA_FIREWALLED_ACK_RES";  // = 0x59, // (null)
        case KADEMLIA_FINDBUDDY_RES:
            return "KADEMLIA_FINDBUDDY_RES";  // = 0x5A  // <TCPPORT (sender) [2]>
        default:
            break;
    };
