...
namespace libed2k {
void session::init(const fingerprint& id, const char* listen_interface, const session_settings& settings) {
...
void session::make_transfer_parameters(const std::string& filepath) { m_impl->m_tpm.make_transfer_params(filepath); }

void session::cancel_transfer_parameters(const std::string& filepath) {
    m_impl->m_tpm.cancel_transfer_params(filepath);
}

void session::start_natpmp() { m_impl->m_io_service.post(boost::bind(&aux::session_impl::start_natpmp, m_impl)); }

void session::start_upnp() { m_impl->m_io_service.post(boost::bind(&aux::session_impl::start_upnp, m_impl)); }
...
void session::stop_dht() { m_impl->m_io_service.post(boost::bind(&aux::session_impl::stop_dht, m_impl)); }
