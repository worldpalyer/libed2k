...
inline piece_block mk_block(const peer_request& r) { return piece_block(r.piece, r.start / BLOCK_SIZE); }
...
inline size_t offset_in_block(const peer_request& r) { return r.start % BLOCK_SIZE; }
...
pending_block::pending_block(const piece_block& b, size_type fsize)
    : skipped(0),
      ...
      busy(false),
      block(b),
      data_size(block_size(b, fsize)),
      data_left(block_range(b.piece_index, b.block_index, fsize)),
      buffer(NULL),
      create_time(time_now()) {}

peer_connection::peer_connection(aux::session_impl& ses, boost::weak_ptr<transfer> t, boost::shared_ptr<tcp::socket> s,
...
peer_connection::peer_connection(aux::session_impl& ses, boost::shared_ptr<tcp::socket> s,
                                 const ip::tcp::endpoint& remote, peer* peerinfo)
    : base_connection(ses, s, remote),
      m_work(ses.m_io_service),
      m_disk_recv_buffer(ses.m_disk_thread, 0),
...
void peer_connection::reset() {
    ...
    add_handler(get_proto_pair<client_ext_hello_answer>(),
                boost::bind(&peer_connection::on_ext_hello_answer, this, _1));
    ...
    add_handler(/*OP_FILEDESC*/ get_proto_pair<client_file_description>(),
                boost::bind(&peer_connection::on_file_description, this, _1));
    ...
    add_handler(
        /*OP_FILEREQANSNOFIL*/ get_proto_pair<client_no_file>(), boost::bind(&peer_connection::on_no_file, this, _1));
    add_handler(/*OP_FILESTATUS*/ get_proto_pair<client_file_status>(),
                boost::bind(&peer_connection::on_file_status, this, _1));
    add_handler(/*OP_HASHSETREQUEST*/ get_proto_pair<client_hashset_request>(),
                boost::bind(&peer_connection::on_hashset_request, this, _1));
    add_handler(/*OP_HASHSETANSWER*/ get_proto_pair<client_hashset_answer>(),
                boost::bind(&peer_connection::on_hashset_answer, this, _1));
    add_handler(/*OP_STARTUPLOADREQ*/ get_proto_pair<client_start_upload>(),
                boost::bind(&peer_connection::on_start_upload, this, _1));
    add_handler(/*OP_QUEUERANKING*/ get_proto_pair<client_queue_ranking>(),
                boost::bind(&peer_connection::on_queue_ranking, this, _1));
    add_handler(std::make_pair(OP_ACCEPTUPLOADREQ, OP_EDONKEYPROT),
                boost::bind(&peer_connection::on_accept_upload, this, _1));
    add_handler(
        /*OP_OUTOFPARTREQS*/ get_proto_pair<client_out_parts>(), boost::bind(&peer_connection::on_out_parts, this, _1));
    add_handler(std::make_pair(OP_CANCELTRANSFER, OP_EDONKEYPROT),
                boost::bind(&peer_connection::on_cancel_transfer, this, _1));
    ...
    add_handler(/*OP_END_OF_DOWNLOAD*/ get_proto_pair<client_end_download>(),
                boost::bind(&peer_connection::on_end_download, this, _1));

    // shared files request and answer
    add_handler(/*OP_ASKSHAREDFILES*/ get_proto_pair<client_shared_files_request>(),
                boost::bind(&peer_connection::on_shared_files_request, this, _1));
    add_handler(/*OP_ASKSHAREDDENIEDANS*/ get_proto_pair<client_shared_files_denied>(),
                boost::bind(&peer_connection::on_shared_files_denied, this, _1));
    add_handler(/*OP_ASKSHAREDFILESANSWER*/ get_proto_pair<client_shared_files_answer>(),
                boost::bind(&peer_connection::on_shared_files_answer, this, _1));

    // shared directories
    add_handler(get_proto_pair<client_shared_directories_request>(),
                boost::bind(&peer_connection::on_shared_directories_request, this, _1));
    add_handler(get_proto_pair<client_shared_directories_answer>(),
                boost::bind(&peer_connection::on_shared_directories_answer, this, _1));

    // shared files in directory
    add_handler(get_proto_pair<client_shared_directory_files_request>(),
                boost::bind(&peer_connection::on_shared_directory_files_request, this, _1));
    add_handler(get_proto_pair<client_shared_directory_files_answer>(),
                boost::bind(&peer_connection::on_shared_directory_files_answer, this, _1));

    // ismod collections
    add_handler(get_proto_pair<client_directory_content_request>(),
                boost::bind(&peer_connection::on_ismod_files_request, this, _1));
    add_handler(get_proto_pair<client_directory_content_result>(),
                boost::bind(&peer_connection::on_ismod_directory_files, this, _1));
    // clients talking
    add_handler(
        /*OP_MESSAGE*/ get_proto_pair<client_message>(), boost::bind(&peer_connection::on_client_message, this, _1));
    add_handler(/*OP_CHATCAPTCHAREQ*/ get_proto_pair<client_captcha_request>(),
                boost::bind(&peer_connection::on_client_captcha_request, this, _1));
    add_handler(/*OP_CHATCAPTCHARES*/ get_proto_pair<client_captcha_result>(),
                boost::bind(&peer_connection::on_client_captcha_result, this, _1));
    add_handler(/*OP_PUBLICIP_RE*/ get_proto_pair<client_public_ip_request>(),
                boost::bind(&peer_connection::on_client_public_ip_request, this, _1));
...
unsigned short peer_connection::user_port() const { return m_active ? m_remote.port() : m_options.m_nPort; }
...
void peer_connection::second_tick(int tick_interval_ms) {
    ...
    if (t->eager_mode() || t->num_free_blocks() == 0) abort_expired_requests();
...
int peer_connection::request_upload_bandwidth(bandwidth_channel* bwc1, bandwidth_channel* bwc2, bandwidth_channel* bwc3,
    ...
    return m_ses.m_upload_rate.request_bandwidth(
        ...
        std::max(m_send_buffer.size(), m_statistics.upload_rate() * 2 / (1000 / m_ses.m_settings.tick_interval)),
        priority, bwc1, bwc2, bwc3, bwc4);
}

int peer_connection::request_download_bandwidth(bandwidth_channel* bwc1, bandwidth_channel* bwc2,
    ...
    return m_ses.m_download_rate.request_bandwidth(
        self_as<peer_connection>(),
        std::max(std::max(outstanding, m_recv_req.length - m_recv_pos),
...
bool peer_connection::has_download_bandwidth() {
    ...
    if (m_quota[download_channel] == 0 && !m_connecting) {
        ...
        int ret =
            request_download_bandwidth(&m_ses.m_download_channel, t ? &t->m_bandwidth_channel[download_channel] : 0,
...
bool peer_connection::has_upload_bandwidth() {
    ...
    if (m_quota[upload_channel] == 0 && !m_send_buffer.empty() && !m_connecting) {
        ...
        int ret = request_upload_bandwidth(&m_ses.m_upload_channel, t ? &t->m_bandwidth_channel[upload_channel] : 0,
...
void peer_connection::request_block() {
    ...
    int num_requests = m_desired_queue_size - (int)m_download_queue.size() - (int)m_request_queue.size();
    ...
    p.pick_pieces(m_remote_pieces, interesting_pieces, num_requests, prefer_whole_pieces, m_peer, state,
                  picker_options(), std::vector<int>(), t->num_peers());
    ...
    bool dont_pick_busy_blocks = (p.num_have() + p.get_download_queue().size() < t->num_pieces()) ||
    ...
    for (std::vector<piece_block>::iterator i = interesting_pieces.begin(); i != interesting_pieces.end(); ++i) {
...
bool peer_connection::add_request(const piece_block& block, int flags) {
    ...
    if (!t->picker().mark_as_downloading(block, get_peer(), state)) return false;
...
void peer_connection::send_block_requests() {
    ...
    if (t->state() == transfer_status::checking_files || t->state() == transfer_status::checking_resume_data ||
        t->state() == transfer_status::downloading_metadata || t->state() == transfer_status::allocating)
        return;
    ...
    if (!rp.empty()) write_request_parts(rp);
}
    ...
    for (std::vector<pending_block>::iterator pi = reqs.begin(); pi != reqs.end();) {                 \
        if (now - pi->create_time > seconds(settings.block_request_timeout)) {                        \
            ...
            DBG("abort expired block request: "                                                       \
                "{piece: "                                                                            \
                << b.piece_index << ", block: " << b.block_index << ", remote: " << m_remote << "}"); \
...
bool peer_connection::requesting(const piece_block& b) const {
    return std::find_if(m_download_queue.begin(), m_download_queue.end(), has_block(b)) != m_download_queue.end() ||
           std::find_if(m_request_queue.begin(), m_request_queue.end(), has_block(b)) != m_request_queue.end();
}

size_t peer_connection::num_requesting_busy_blocks() const {
    size_t res = 0;

    for (std::vector<pending_block>::const_iterator i = m_download_queue.begin(), end(m_download_queue.end()); i != end;
         ++i) {
        if (i->busy) ++res;
    }

    for (std::vector<pending_block>::const_iterator i = m_request_queue.begin(), end(m_request_queue.end()); i != end;
         ++i) {
        if (i->busy) ++res;
    }
...
int peer_connection::outstanding_bytes() const {
    ...
    if (t) {
        size_type s = t->size();
        for (std::vector<pending_block>::const_iterator i = m_download_queue.begin(), end(m_download_queue.end());
             i != end; ++i) {
...
bool peer_connection::allocate_z_receive_buffer() {
    if (!m_z_recv_buffer) m_z_recv_buffer = m_ses.allocate_z_buffer();
...
void peer_connection::on_sent(const error_code& error, std::size_t bytes_transferred) {
    ...
    if (!m_payloads.empty()) {
        for (std::vector<range>::iterator i = m_payloads.begin(); i != m_payloads.end(); ++i) {
    ...
    m_payloads.erase(std::remove_if(m_payloads.begin(), m_payloads.end(), range_below_zero), m_payloads.end());
...
void peer_connection::disconnect(error_code const& ec, int error) {
    ...
    for (aux::session_impl::transfer_map::const_iterator i = m_ses.m_transfers.begin(), end(m_ses.m_transfers.end());
         i != end; ++i)
...
void peer_connection::connect(int ticket) {
    ...
    m_socket->async_connect(m_remote, boost::bind(&peer_connection::on_connect, self_as<peer_connection>(), _1));
}
...
void peer_connection::fast_reconnect(bool r) {
    if (!get_peer() || get_peer()->fast_reconnects > 1) return;
    m_fast_reconnect = r;
    get_peer()->last_connected = m_ses.session_time();
    int rewind = m_ses.settings().min_reconnect_time * m_ses.settings().max_failcount;
    if (get_peer()->last_connected < rewind)
        get_peer()->last_connected = 0;
    else
        get_peer()->last_connected -= rewind;

    if (get_peer()->fast_reconnects < 15) ++get_peer()->fast_reconnects;
}
...
void peer_connection::on_error(const error_code& error) { disconnect(error); }

bool peer_connection::can_write() const {
    // if we have requests or pending data to be sent or announcements to be made
    // we want to send data
    return !m_send_buffer.empty() && m_quota[upload_channel] > 0 && !m_connecting;
}
...
const std::vector<pending_block>& peer_connection::download_queue() const { return m_download_queue; }

const std::vector<pending_block>& peer_connection::request_queue() const { return m_request_queue; }
...
bool peer_connection::has_hash(const md4_hash& hash) const { return (m_hClient == hash); }

bool peer_connection::operator==(const net_identifier& np) const { return has_network_point(np); }

bool peer_connection::operator==(const md4_hash& hash) const { return has_hash(hash); }

void peer_connection::send_message(const std::string& strMessage) {
    DBG("send message: " << strMessage << " ==> " << m_remote);
    m_ses.m_io_service.post(boost::bind(&peer_connection::send_throw_meta_order<client_message>,
...
void peer_connection::request_shared_files() {
    ...
    m_ses.m_io_service.post(boost::bind(&peer_connection::send_throw_meta_order<client_shared_files_request>,
...
void peer_connection::request_shared_directories() {
    ...
    m_ses.m_io_service.post(boost::bind(&peer_connection::send_throw_meta_order<client_shared_directories_request>,
...
void peer_connection::request_shared_directory_files(const std::string& strDirectory) {
    ...
    m_ses.m_io_service.post(boost::bind(&peer_connection::send_throw_meta_order<client_shared_directory_files_request>,
                                        self_as<peer_connection>(),
                                        client_shared_directory_files_request(strDirectory)));
}
...
void peer_connection::request_ismod_directory_files(const md4_hash& hash) {
    ...
    m_ses.m_io_service.post(boost::bind(&peer_connection::send_throw_meta_order<client_directory_content_request>,
...
void peer_connection::send_data(const peer_request& req) {
    ...
    if (r.length > 0) {
        t->filesystem().async_read(
            r, boost::bind(&peer_connection::on_disk_read_complete, self_as<peer_connection>(), _1, _2, r, left));
...
void peer_connection::on_disk_read_complete(int ret, disk_io_job const& j, peer_request r, peer_request left) {
    boost::mutex::scoped_lock l(m_ses.m_mutex);
...
void peer_connection::receive_data() {
    ...
    if (!can_read(&m_channel_state[download_channel])) {
        DBG((boost::format("cannot read: {disk_queue_limit: %1%}") % m_ses.settings().max_queued_disk_bytes).str());
    ...
    if (b == m_download_queue.end()) {
        ERR("The block incoming from " << m_remote << " : {piece: " << block.piece_index << ", block: "
                                       << block.block_index << ", length: " << block_size(block, t->size())
                                       << "} was not in the request queue");
        skip_data();
        return;
    }

    if (!b->buffer && !(b->buffer = allocate_receive_buffer(block_size(block, t->size())))) return;
    ...
    if (max_receive > 0) {
        ...
        boost::asio::async_read(
            *m_socket, boost::asio::buffer(b->buffer + offset_in_block(m_recv_req) + m_recv_pos, max_receive),
            make_read_handler(boost::bind(&peer_connection::on_receive_data, self_as<peer_connection>(), _1, _2)));
    } else {
        do_read();
    }
}
...
void peer_connection::on_receive_data(const error_code& error, std::size_t bytes_transferred) {
    ...
    if (b == m_download_queue.end()) {
        ERR("The block we just got from "
            << m_remote << " : {piece: " << block_finished.piece_index << ", block: " << block_finished.block_index
            << ", length: " << block_size(block_finished, t->size()) << "} was not in the request queue");
        skip_data();
        return;
    }
    ...
    if (picker.is_downloaded(block_finished)) {
        DBG("The block we just got from "
            << m_remote << " : {piece: " << block_finished.piece_index << ", block: " << block_finished.block_index
            << ", length: " << block_size(block_finished, t->size()) << "} is already downloaded");
    ...
    if (m_recv_pos == m_recv_req.length) {
        if (complete_block(*b)) {
            ...
            fs.async_write(req, holder, boost::bind(&peer_connection::on_disk_write_complete,
            ...
            if (picker.is_piece_finished(m_recv_req.piece) && !was_finished) {
                ...
                t->async_verify_piece(m_recv_req.piece, hash,
                                      boost::bind(&transfer::piece_finished, t, m_recv_req.piece, _1));
            }
        }
...
void peer_connection::skip_data() {
    ...
    m_socket->async_read_some(
        ...
        make_read_handler(boost::bind(&peer_connection::on_skip_data, self_as<peer_connection>(), _1, _2)));
}
...
void peer_connection::on_disk_write_complete(int ret, disk_io_job const& j, peer_request req,
                                             boost::shared_ptr<transfer> t) {
    boost::mutex::scoped_lock l(m_ses.m_mutex);
...
void peer_connection::append_misc_info(tag_list<boost::uint32_t>& t) {
    ...
    t.add_tag(make_typed_tag(make_full_ed2k_version(SO_AMULE, m_ses.settings().mod_major, m_ses.settings().mod_minor,
                                                    m_ses.settings().mod_build),
                             CT_EMULE_VERSION, true));
...
void peer_connection::write_hello() {
    ...
    client_hello hello(m_ses.settings().user_agent,
                       ...
                       m_ses.settings().client_name, m_ses.settings().mod_name, m_ses.settings().m_version);
...
void peer_connection::write_file_answer(const md4_hash& file_hash, const std::string& filename) {
...
void peer_connection::write_file_status(const md4_hash& file_hash, const bitfield& status) {
...
void peer_connection::write_hashset_answer(const md4_hash& file_hash, const std::vector<md4_hash>& hash_set) {
...
void peer_connection::on_hello(const error_code& error) {
    if (!error) {
        ...
        DBG("hello {"
            << " server point = " << hello.m_server_network_point << " network point = " << hello.m_network_point
            << "} <== " << m_remote);
...
void peer_connection::on_hello_answer(const error_code& error) {
    if (!error) {
        ...
        DBG("hello answer {name: " << m_options.m_strName << " : mod name: " << m_options.m_strModVersion
                                   << ", port: " << m_options.m_nPort << "} <== " << m_remote);

        m_ses.m_alerts.post_alert_should(peer_connected_alert(get_network_point(), get_connection_hash(), m_active));
        finalize_handshake();
    } else {
...
void peer_connection::on_file_status(const error_code& error) {
    if (!error) {
        ...
        if (fs.m_status.size() == 0) fs.m_status.resize(t->num_pieces(), 1);

        DBG("file status answer " << fs.m_hFile << ", [" << bitfield2string(fs.m_status) << "] <== " << m_remote);
...
std::vector<std::string> dirlist(const DirContainer& dirs) {
    ...
    for (size_t i = 0; i < dirs.m_collection.size(); ++i) res.push_back(dirs.m_collection[i].m_collection);
    return res;
}
...
std::vector<std::string> filelist(const FileContainer& files) {
    ...
    for (size_t i = 0; i < files.m_collection.size(); ++i)
        for (size_t j = 0; j < files.m_collection[i].m_list.size(); ++j) {
            ...
            if (ptag->getNameId() == FT_FILENAME) res.push_back(ptag->asString());
        }
    return res;
}
...
void peer_connection::on_shared_files_request(const error_code& error) {
    if (!error) {
        ...
        if (m_ses.settings().m_show_shared_files) {
            ...
            DBG("shared files: " << boost::algorithm::join(filelist(sfa.m_files), ", ") << " ==> " << m_remote);
...
void peer_connection::on_ismod_files_request(const error_code& error) {
    if (!error) {
        ...
        if (boost::shared_ptr<transfer> ct = m_ses.find_transfer(req.m_hash).lock()) {
            ...
            for (std::deque<emule_collection_entry>::const_iterator i = coll.m_files.begin(); i != coll.m_files.end();
                 ++i) {
                boost::shared_ptr<transfer> t = m_ses.find_transfer(i->m_filehash).lock();
                if (t) ans.m_files.m_collection.push_back(t->get_announce());
            }

            DBG("ismod directory content: {hash: " << ans.m_hdirectory << ", files: ["
                                                   << boost::algorithm::join(filelist(ans.m_files), ", ")
                                                   << "]} <== " << m_remote);
...
void peer_connection::on_shared_files_answer(const error_code& error) {
    if (!error) {
        ...
        DBG("shared files: " << boost::algorithm::join(filelist(packet.m_files), ", ") << " <== " << m_remote);
...
void peer_connection::on_shared_directories_answer(const error_code& error) {
    if (!error) {
        ...
        DBG("shared directories: " << boost::algorithm::join(dirlist(sd.m_dirs), ", ") << " <== " << m_remote);
...
void peer_connection::on_shared_directory_files_request(const error_code& error) {
    if (!error) {
        ...
        DBG("request shared directory files: {dir: " << req.m_directory.m_collection << "} <== " << m_remote);
        for (aux::session_impl::transfer_map::const_iterator i = m_ses.m_transfers.begin(),
                                                             end(m_ses.m_transfers.end());
             i != end; ++i) {
            if (collection_dir(i->second->name()) == req.m_directory.m_collection) {
                ...
                DBG("shared directory files: {dir: " << ans.m_directory.m_collection << ", files: ["
                                                     << boost::algorithm::join(filelist(ans.m_list), ", ") << "]} ==> "
                                                     << m_remote);
                send_throw_meta_order(ans);
            }
        }
...
void peer_connection::on_shared_directory_files_answer(const error_code& error) {
    if (!error) {
        ...
        DBG("shared directory files: {dir: " << sdf.m_directory.m_collection << ", files: ["
                                             << boost::algorithm::join(filelist(sdf.m_list), ", ")
                                             << "]} <== " << m_remote);
        m_ses.m_alerts.post_alert_should(shared_directory_files_alert(get_network_point(), get_connection_hash(),
                                                                      sdf.m_directory.m_collection, sdf.m_list));
    } else {
        ERR("shared directories answer error " << error.message() << " <== " << m_remote);
    }
}
...
void peer_connection::on_ismod_directory_files(const error_code& error) {
    if (!error) {
        ...
        DBG("ismod directory files: {dir: " << cdcr.m_hdirectory << ", files: ["
                                            << boost::algorithm::join(filelist(cdcr.m_files), ", ")
                                            << "]} <== " << m_remote);
        m_ses.m_alerts.post_alert_should(ismod_shared_directory_files_alert(get_network_point(), get_connection_hash(),
                                                                            cdcr.m_hdirectory, cdcr.m_files));
    } else {
        ERR("shared directories answer error " << error.message() << " <== " << m_remote);
    }
}
...
void peer_connection::on_request_parts(const error_code& error) {
    if (!error) {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            if (reqs.empty()) {
                ERR("incorrect request [" << rp.m_begin_offset[i] << ", " << rp.m_end_offset[i] << "] from "
                                          << m_remote);
            }
        }
        fill_send_buffer();
...
void peer_connection::on_compressed_part(const error_code& error) {
    if (!error) {
        ...
        DBG("compressed part " << sp.m_hFile << " [" << begin_offset << ", " << end_offset << "]"
                               << " <== " << m_remote);
...
template <typename T>
void peer_connection::defer_write(const T& t) {
    m_deferred.push_back(make_message(t));
}
...
bool peer_connection::complete_block(pending_block& b) {
    ...
    if (m_recv_compressed && b.completed()) {
        ...
        int rc = mz_uncompress((Bytef*)m_disk_recv_buffer.get(), &size, (const Bytef*)m_z_recv_buffer, b.data_size);
        if (rc != Z_OK) {
